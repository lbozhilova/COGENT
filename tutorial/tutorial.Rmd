...
There are a number of ways such networks can be built from gene expression data across multiple samples. The easiest might be to calculate a Pearson correlation coefficient between the expression profiles of every pair of genes, and then build edges where the correlations are high (say, take the tio 10% of values). Various choices of similarity metrics and thresholds are possible here, and there exist more complex network construction approaches. It is not always clear which to apply.

In order to help motivate data-driven approaches to network construction, we developed the `R` package **COGENT** (**C**onsistency of **G**ene **E**xpression **N**e**T**works). COGENT provides a suite of assessment tools for gene co-expression network construction. It can be used to choose between different network construction approaches, as well as to inform threshold choice. The main idea behind COGENT is that a good network construction function should produce similar networks when only a subset of the available expression samples are used. 
...
As meantioned above, perhaps the easiest way to build a co-expression network is by calculating Pearson correlation coefficients and thresholding. Here is a simple function, which will take the top 10% of correlation values.
...
Notice the difference between the global similarity (0.56) and the degree similarity, measured by Pearson's correlation coefficients (0.93). While the generated random networks are considerably different, their degree sequences are very similar. If subsequent analysis will be based on the degrees, a relatively low global similarity may be unimportant. 
...
```{r}
stabilityPearson <- cogentLinear(df=yeastData, netwkFun=buildPearson, propShared=0.50, repCount=100,
                              nodeFun=calculateDegrees, nodeModes="all",
...
```{r}
stabilityKendall <- cogentParallel(df=yeastData, netwkFun=buildKendall, propShared=0.50, repCount=100, 
                                   threadCount=4, nodeFun=calculateDegrees, nodeModes="all",
                                   use="pairwise.complete.obs", method="pearson", k.or.p=0.10, scale=TRUE)
...
Again, we can apply it multiple times, say 100 each, at several thresholds. Here we parallelise explicitly. We threshold for network density between 0.01 and 0.30 at 0.01 intervals. This corresponds to quantiles of the correlation distribution between 0.70 and 0.99.
...
```{r}
thresholdComparisonDF <- subset(thresholdComparisonDF, !is.na(thresholdComparisonDF$correctedSimilarity))
...
In addition to gene expression data, COGENT can be readily applied to other types of data, where co-occurrence signifies a functional relationship of interest. This could, for example, include microbiote data, as well as discrete data, suchas syntehtic lethality screens.
